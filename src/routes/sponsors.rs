...
use crate::{
    prisma::{ hackathon::UniqueWhereParam, sponsor::{ self, Data } },
...
#[axum::debug_handler]
pub async fn get_all_sponsors(State(app_state): State<AppState>) -> Result<
    Json<Vec<Data>>,
    String
> {
    match
...
pub async fn sponsor_get_router() -> Router {
    ...
    Router::new().route("/", post(create_sponsor).get(get_all_sponsors)).with_state(state)
}
